...
def sentence3() -> Expr:
    ...
    c1 = PacmanAlive_1 % ((PacmanAlive_0 & (~PacmanKilled_0))
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    # print("sensorModel = ", sensorModel)
    # print("successorAxioms = ", successorAxioms)
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    KB.append(pacphysicsAxioms(0, all_coords,
              non_outer_wall_coords, walls_grid, None))
    ...
    KB_Expr = conjoin(KB)
    model1 = findModel(
        conjoin([KB_Expr, PropSymbolExpr(pacman_str, x1, y1, time=1)])
    )
    model2 = findModel(
        conjoin([KB_Expr, ~PropSymbolExpr(pacman_str, x1, y1, time=1)])
    )
...
def positionLogicPlan(problem) -> List:
    ...
    while True:
        ...
        KB.append(pacmanSuccessorAxiomSingle(x0, y0, t+1, walls_grid))
        KB.extend([pacmanSuccessorAxiomSingle(x, y, t+1, walls_grid)
...
def foodLogicPlan(problem) -> List:
    ...
    while True:
        ...
        if model:
            return extractActionSequence(model, actions)
        KB.extend([pacmanSuccessorAxiomSingle(x, y, t+1, walls)
                   for x, y in non_wall_coords])
        food_eaten = [
            (~PropSymbolExpr(food_str, x, y, time=t) | PropSymbolExpr(pacman_str, x, y, time=t)) % ~PropSymbolExpr(
                food_str, x, y, time=t+1)
            for x, y in food
        ]
        KB.extend(food_eaten)
        t += 1
# ______________________________________________________________________________
# QUESTION 6
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            ...
            if findModel(conjoin(KB) & pacman_position):
                possible_locations.append((x, y))
            if entails(conjoin(KB), pacman_position):
                KB.append(pacman_position)
            elif entails(conjoin(KB), ~pacman_position):
                KB.append(~pacman_position)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for (x, y) in non_outer_wall_coords:
            ...
            if entails(conjoin(KB), wall_position):
                KB.append(wall_position)
                known_map[x][y] = 1
            elif entails(conjoin(KB), ~wall_position):
                KB.append(~wall_position)
                known_map[x][y] = 0
