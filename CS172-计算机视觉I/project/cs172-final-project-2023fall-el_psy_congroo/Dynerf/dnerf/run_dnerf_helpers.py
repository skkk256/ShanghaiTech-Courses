...
class DirectTemporalNeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, input_ch_time=1, output_ch=4, skips=[4],
                 use_viewdirs=False, memory=[], embed_fn=None, zero_canonical=True, args=None):
        ...
        # Create demormation net
        self.deform_net = deform_network(args).to('cuda')
    ...
    def forward(self, x, ts):
        ...
        if cur_time == 0. and self.zero_canonical:
            ...
            _, _, _, _, _, dx = self.deform_net(input_pts, self._time, self._time_out)
