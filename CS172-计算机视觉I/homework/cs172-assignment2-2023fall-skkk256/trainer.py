...
from modules.kitti_utils import *
...
class Trainer:
    def __init__(self, options):
        ...
        if self.opt.feature_extractor == "featdepth":
            self.models["feature_encoder"], _ = build_extractor(50, "models/autoencoder.pth", encoder_only=True)
            self.models["feature_encoder"].to(self.device)
        ...
        if self.opt.feature_extractor == "featdepth":
            self.backproject_feature = BackprojectDepth(self.opt.batch_size, int(self.opt.height/2), int(self.opt.width/2))
            self.backproject_feature.to(self.device)
            self.project_3d_feature = Project3D(self.opt.batch_size, int(self.opt.height/2), int(self.opt.width/2))
            self.project_3d_feature.to(self.device)
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                # ------------------------------------------------------------------------------------------------------
                # 语法：torch.nn.functional.grid_sample(input, grid, mode='bilinear', 
                #                                      padding_mode='zeros', align_corners=None)
                
                # 详解参考： https://blog.csdn.net/weixin_42973210/article/details/132315563
                # ------------------------------------------------------------------------------------------------------
    ...
    def generate_features_pred(self, inputs, outputs):
        ...
        # outputs["disp"]直接输出的就是视差图，并且仍然多尺度[0,1,2,3]分布。
        disp = outputs[("disp", 0)]
        disp = F.interpolate(disp, [self.opt.height//2, self.opt.width//2], mode="bilinear", align_corners=False)
        source_scale = 0
        # 将disp值映射到[0.01,10]，并求倒数就能得到深度值
        _, depth = disp_to_depth(disp, self.opt.min_depth, self.opt.max_depth)
        ...
        for _, frame_id in enumerate(self.opt.frame_ids[1:]):
            ...
            pix_coords = self.project_3d_feature(cam_points, K, T)  # [b,h,w,2]
                
            img = inputs[("color", frame_id, 0)]
            src_f = self.models["feature_encoder"](img)[0]
        ...
        for scale in self.opt.scales:
            ...
            if self.opt.feature_extractor == "featdepth":   
                for frame_id in self.opt.frame_ids[1:]:
                    ...
                    tgt_f = self.models["feature_encoder"](inputs[("color", 0, 0)])[0]
